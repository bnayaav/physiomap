...
export default function App() {
  const [layers, setLayers] = useState({
    skeleton: true, muscles: false, arteries: false,
    veins: false, nerves: false, points: true,
  });
  ...
  const PointInfo = () => !point ? (
    <div className="empty-state">
      ...
      <div className="empty-sub">לחץ על עיגול צבעוני בדגם הגוף</div>
    </div>
  ) : (
    <div className="point-info">
      ...
      <div>
        ...
        <div className="pressure-bar">
          {[1,2,3,4,5,6].map(i => (
            <div key={i} className="pressure-seg"
              style={{ background: i <= pressureLevel ? point.color : undefined,
                       borderColor: i <= pressureLevel ? 'transparent' : undefined }} />
          ))}
        </div>
      </div>
      <div className="tags-row">
        {point.tags.map(t => (
          <span key={t} className="tag-badge"
            style={{ borderColor: point.color + '66', color: point.color }}>{t}</span>
        ))}
      </div>
      ...
      <div className="notes-section">
        ...
        <textarea className="notes-input"
          placeholder={"הסבר למטופל על נקודה " + point.id + "..."}
          value={noteInput}
    ...
    <>
      <div className="sidebar-section">
        ...
        <div className="toggle-group">
          <button className={"toggle-btn" + (gender === 'male' ? ' active' : '')}
            onClick={() => setGender('male')}>♂ גבר</button>
          <button className={"toggle-btn" + (gender === 'female' ? ' active' : '')}
            onClick={() => setGender('female')}>♀ אישה</button>
        </div>
      </div>
      <div className="sidebar-section">
        <div className="section-title">מערכות גוף</div>
        {Object.entries(LAYERS_CONFIG).map(([key, cfg]) => (
          <button key={key} className={"layer-btn" + (layers[key] ? ' active' : '')}
            ...
            <span className="layer-check">{layers[key] ? '✓' : ''}</span>
          </button>
        ))}
      </div>
      <div className="sidebar-section">
        ...
        <div className="quick-points">
          {Object.values(PRESSURE_POINTS).map(p => (
            <button key={p.id}
              ...
              onClick={() => { handlePointClick(p.id); setShowSidebar(false); }}>
              {p.id}
            </button>
          ))}
        </div>
    ...
    <div className="app" dir="rtl">
      <header className="header">
        <div className="logo">
          ...
          <span className="logo-sub">מפת גוף</span>
        </div>
        <div className="header-actions">
          <input className="patient-input" type="text"
            placeholder="שם המטופל..."
            value={patientName}
            onChange={e => setPatientName(e.target.value)} />
          <button className="mob-menu-btn" onClick={() => setShowSidebar(true)}>☰</button>
        </div>
      </header>
        ...
        <div className="canvas-area">
          <div className="canvas-bg">
            <div className="zoom-wrap" style={{ transform: 'scale(' + zoom + ')', transformOrigin: 'center top' }}>
              <BodySVG layers={layers} selectedPoint={selectedPoint}
                onPointClick={handlePointClick} gender={gender} view="front" />
            </div>
          </div>
          ...
          {selectedPoint && (
            <button className="mob-info-btn" onClick={() => setShowInfo(true)}>
              פרטי נקודה {selectedPoint} ↑
            </button>
          )}
        </div>
      ...
      {showSidebar && (
        <div className="drawer-overlay" onClick={() => setShowSidebar(false)}>
          <div className="drawer drawer-right" onClick={e => e.stopPropagation()}>
            <div className="drawer-header">
              <span>הגדרות</span>
              <button className="drawer-close" onClick={() => setShowSidebar(false)}>✕</button>
            </div>
            <div className="drawer-body"><SidebarContent /></div>
          </div>
        </div>
      )}

      {showInfo && (
        <div className="drawer-overlay" onClick={() => setShowInfo(false)}>
          <div className="drawer drawer-bottom" onClick={e => e.stopPropagation()}>
            <div className="drawer-handle-bar" />
            <div className="drawer-header">
              <span>{point ? point.id : 'נקודת לחיצה'}</span>
              <button className="drawer-close" onClick={() => setShowInfo(false)}>✕</button>
            </div>
            <div className="drawer-body"><PointInfo /></div>
          </div>
        </div>
      )}
    </div>
